...
Email)
VALUES
  ('Riya','Sharma',1,55000,'2024-02-02','mohit.desai@example.com');
...
INSERT INTO Departments (DepartmentName, Location, HeadOfDepartment, AnnualBudget) 
VALUES  
('Researsh', 'Hyderabad', 'Dr. Meera Joshi', 2000000) ;   
...
-- A client requires a project lead for their team. Find the employee details where the 
-- name starts with 'S'. 
SELECT * FROM Employees
...
SELECT * FROM Employees
WHERE DateOfJoining >='2024-02-01' AND DateOfJoining < '2024-03-01';
...
SELECT * FROM Employees
WHERE FirstName = '%Manager%' OR LastName = '%Manager%';
...
-- Marketing expansion is being planned. Find all departments whose names start 
-- with "M". 
SELECT * FROM Departments
...
SELECT d.DepartmentName,SUM(e.Salary) AS TotalSalaryPaid
FROM Employees e
JOIN Departments d
ON d.DepartmentID = e.DepartmentID
GROUP BY d.DepartmentName;

-- For employee retention analysis, count the number of employees in each 
-- department. 
SELECT d.DepartmentName,COUNT(e.EmployeeID) AS CountOfEmp
FROM Employees e
JOIN Departments d
ON d.DepartmentID = e.DepartmentID
GROUP BY d.DepartmentName;

-- The finance team wants to understand salary variation. Find the average salary in 
-- each department. 
SELECT d.DepartmentName,AVG(e.Salary) AS CountOfEmp
FROM Employees e
JOIN Departments d
ON d.DepartmentID = e.DepartmentID
GROUP BY d.DepartmentName;

-- To reward experienced employees, identify the earliest joining date in each 
-- department. 
SELECT d.DepartmentName,MAX(e.DateOfJoining) AS EarliestDateOfJoining
FROM Employees e
JOIN Departments d
ON d.DepartmentID = e.DepartmentID
GROUP BY d.DepartmentName;
...
-- The HR team is preparing performance reports. Retrieve the maximum salary in 
-- each department. 
...
-- The finance team wants to review high-salary employees. Find the total salary paid 
...
ON d.DepartmentID = e.DepartmentID
GROUP BY d.DepartmentName
HAVING TotalSalaryPaid > 60000;

-- To monitor departmental growth, count the number of employees who joined in 
-- 2024 in each department. 
SELECT d.DepartmentName,COUNT(*)
FROM Employees e
JOIN Departments d
ON d.DepartmentID = e.DepartmentID
WHERE e.DateOfJoining BETWEEN '2024-01-01' AND '2024-12-31'
GROUP BY d.DepartmentName;

-- For training allocation, count the number of employees with salaries between 
...
-- For diversity analysis, count the number of employees whose names start with 
-- each letter of the alphabet. 
SELECT 
    LEFT(FirstName, 1) AS StartingLetter,
    COUNT(*) AS EmployeeCount
FROM Employees
WHERE LEFT(FirstName, 1) BETWEEN 'A' AND 'Z'
GROUP BY LEFT(FirstName, 1)
ORDER BY StartingLetter;


-- For city-based analysis, count the number of departments located in each city. 
SELECT Location,COUNT(*) AS CountOfDepartment
FROM Departments
GROUP BY Location;

-- To manage high-budget projects, find the total budget allocated to all departments 
-- in each city. 
SELECT Location,SUM(AnnualBudget) AS TotalBudget
FROM Departments
GROUP BY Location;

-- For leadership tracking, count the number of departments headed by each 
-- manager. 
SELECT HeadOfDepartment,COUNT(*) AS NumberOfDepartment
FROM Departments
GROUP BY HeadOfDepartment;

-- To plan budget allocation, calculate the average budget of departments in each 
-- city. 
SELECT Location,AVG(AnnualBudget) AS AVGBudget
FROM Departments
GROUP BY Location;

-- For funding efficiency, find the maximum and minimum budgets among 
...
-- To create a full name column, concatenate the first name and last name of each 
-- employee. 
SELECT concat(FirstName, " ",LastName) AS FullName
FROM Employees;

-- For a project report, concatenate the department name and its location to display a 
-- complete department address. 
SELECT concat(DepartmentName, ", ",Location) AS FullAddress
FROM Departments;

-- To prepare a personalized email greeting, concatenate "Hello " with the employee's '
-- name and a comma. 
SELECT FirstName,concat(" Hello"," ",FirstName, " ",LastName, " , ") AS GreetingwithName
FROM Employees;

-- For creating a unique employee ID, concatenate the department ID and employee 
-- number. 
SELECT concat(DepartmentID, "",EmployeeID) AS UniqueID
FROM Employees;

-- For creating a company contact list, concatenate the employees phone number 
-- with their extension (if applicable). 


-- To display the complete address, concatenate the street, city, and postal code of 
-- each employee. 

-- To create an employee login ID, concatenate the employee’s first name, last name, 
-- and their department ID. 
SELECT FirstName,concat(FirstName, "",LastName,"",DepartmentID) AS LoginID
FROM Employees;

-- For sending out a personalized notification, concatenate the employees department 
-- with their position. 
SELECT CONCAT(HeadOfDepartment, "-",DepartmentName) AS MessageSending
FROM Departments;

-- To display the employees work status, concatenate their job title and current 
-- project. 


-- For an annual report, concatenate the year and employee’s first name to create a 
-- unique report reference code. 
SELECT FirstName,CONCAT(FirstName,"",YEAR(DateOfJoining)) AS referanceCode
FROM Employees;
                              
-- 5] Situational Questions on the UPDATE Clause 

-- As part of an organizational review, we need to update the salary of all employees 
...
-- After the recent promotion evaluation, it has been decided that the employee with 
...
UPDATE Employees
SET JobRole = 'Senior Developer'
WHERE EmployeeID=101;

-- The HR team has informed us of a change in the contact information of employee 
...
-- The company has recently implemented a new email domain. All employees who 
...
UPDATE Employees
SET Email ='@newcompany'
WHERE DateOfJoining > '2023-01-01';

-- It has come to our attention that there was an error in the naming of the HR 
-- department. The department name for DepartmentID 5 needs to be updated from "HR" to 
-- "Human Resources" to reflect the companys official title. Perform this update to ensure 
-- consistency across the records. 
UPDATE Departments
SET DepartmentName='Human Resources'
WHERE DepartmentID=1;

-- As part of a company-wide salary review process, all employees who are currently 
...
-- The Marketing department has recently moved to a new office in a different city. 
-- The location of the Marketing department (DepartmentID: 11) needs to be updated to reflect this 
-- change. Please update the location to "Mumbai" in the departments records accordingly. 
UPDATE Departments
SET Location = 'Mumbai'
WHERE DepartmentID;
...
-- EmployeeID 110, who was previously working on a different project, has now 
...
-- The employee with EmployeeID 103 had a delayed start date due to personal 
-- reasons. We need to update their joining date to the correct date, which is 2024-03-15, to ensure 
-- the records accurately reflect the employees actual start date with the company. 
UPDATE Employees
...
-- Due to an employee leaving the company, we need to delete the record of 
-- EmployeeID 101 from the Employee table. 
DELETE FROM Employees
WHERE EmployeeID=101;

-- The company is undergoing a restructuring. As part of this process, we need to 
...
-- We are cleaning up the database and need to delete employees with a salary lower 
-- than ₹30,000, as they are no longer part of the workforce. 
DELETE FROM Employees
WHERE Salary < 30000;

--  As per a recent audit, employees who joined before 2020 but no longer have any 
-- active projects should be removed from the system. Delete these employee records. 
DELETE FROM Employees
WHERE DateOfJoining < '2020-01-01';

-- After the closure of "Project Beta", we need to delete all records of employees who 
-- were working on this project (assumed to be recorded in ProjectName). 
DELETE FROM Employees
WHERE ProjectName = 'Project Beta';

-- An employee, EmployeeID 135, was added mistakenly and is no longer part of the 
-- organization. Please delete their record from the Employee table. 
DELETE FROM Employees
WHERE EmployeeID = 135;

-- The Marketing department (DepartmentID: 3) has undergone significant layoffs. 
...
-- As part of a data privacy initiative, we need to delete records of employees who 
-- have not provided their email addresses or contact numbers. 
DELETE FROM Employees
WHERE Email;

-- We need to remove all employees whose salary is above ₹1,00,000 but were part of 
-- a temporary contract that has now expired. Delete these employee records. 
DELETE FROM Employees
WHERE Salary < 100000;

-- The company has decided to shut down operations in certain regions. Delete the 
...
-- Due to a change in company policy, we need to rename the Employee table to Staff. 
...
--  
-- As the company has decided to start tracking employees marital status, add a new 
...
--  
-- The company has decided to store the employees date of birth. Add a DateOfBirth 
-- column of type DATE to the Employee table. 
-- Note: Please ensure to retrieve the changes made to check the columns impact on future data 
-- consistency. 
ALTER TABLE Employees
ADD DateOfBirth DATE;
--  
-- We need to change the data type of the Salary column in the Employee table from INT 
...
--  
-- To better categorize employees, we need to add a new column EmployeeCategory in 
...
--  
-- The company wants to keep track of the department name for employees. 
...
--  We need to set the default value of the Status column in the Employee table to 
...
--  
-- As part of a company-wide restructuring, the DepartmentID column in the Employee 
...
--  
-- The company has decided to archive old employee data, so the Employee table needs 
...
--  ALTER TABLE Employee
DROP PRIMARY KEY,
ADD PRIMARY KEY (EmployeeID, JoinDate);


-- We need to create a foreign key constraint between the Employee table and the 
